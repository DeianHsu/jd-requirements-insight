...
def make_extraction(
    extraction_id: int, job_id: int, version: str = "0.9"
) -> JobExtraction:
    """创建一份带抽取器版本身份的抽取主记录。"""
    if version == "0.9":
...
def test_loads_occurrences_from_multiple_jobs_without_field_loss(
    ...
    assert by_id[1].extractor_version == "test-model|prompt:0.10|schema:3.0"
...
def test_extractor_version_wrong_values_are_rejected() -> None:
    ...
    for version in (
        ...
        with pytest.raises(ValueError, match="当前只支持 v0.10"):
            assert_current_extractor_version(version)
...
def test_extractor_version_valid_identity_passes() -> None:
    ...
    assert_current_extractor_version("deepseek-v4-flash|prompt:0.10|schema:3.0")
    assert_current_extractor_version("model|prompt:0.10|schema:3.0|revision:1")


def test_explicit_extraction_version_is_loaded(tmp_path: Path) -> None:
    """验证同一JD并存多个版本时显式指定当前版本装配；旧版本被拒绝。"""
    engine, session_factory = make_database(tmp_path)
    with session_factory() as session:
        session.add(make_job(1, "job-a.md"))
        session.add(make_extraction(1, 1, version="0.9"))
