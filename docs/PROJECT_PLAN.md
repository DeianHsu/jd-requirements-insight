...
| `未开始` | 尚未启动 |
| `进行中` | 主体开发中 |
| `待收口` | 主体实现完成，真实链路验证 / 接口收口 / 历史例外处理未完（未正式关闭） |
| `✅ 已关闭` | 正式关闭，关闭条件全部满足 |
| `🟡 已关闭·含例外` | 正式关闭，但存在明确记录的例外（不阻塞当前基线；见「当前例外」清单） |
...
| P0-7 正式生产主线收口 | 显式数据库目标；模型候选不入正式表；app finalize 定稿；报告只消费完整定稿批次；离线来源审计；真实链路 E2E | 🟡 已关闭·含例外（例外 1 待决；例外 3 命名待办） |

## 当前例外

| # | 所属阶段 | 例外内容 | 处置状态 |
|---|---|---|---|
| 1 | P0-3B / P0-7 | JD 1/2/3 正式抽取记录缺新定稿合同字段（机器分类 `unverified`，保留 legacy 人工审计结论；JD 4/5 为 `fully_bound`） | **待决**：补结构化豁免记录（被豁免记录、原因、证据、批准人、允许消费的下游、是否仅限当前批次、新增数据禁止继续豁免）或重新验收补齐；决策前报告生成显式标注 provenance 风险 |
| 2 | P0-4 | 归并稳定性 positive-pair Jaccard 39~67%（诊断指标）未达标，正式结果靠人工裁决兜底 | 不阻塞当前基线；扩样 5→6→7→8 时逐轮测量新增不稳定对与裁决量，有数据后再定是否调整 Prompt / 引入确定性归一化 |
| 3 | P0-7 | 抽取来源状态 `reviewed_unbound` 命名过乐观（八个绑定字段任一存在即标记） | 非阻塞待办：细分 `fully_bound` / `reviewed_legacy` / `partially_bound` / `unverified` |

## 当前下一步

1. 保持当前 5 JD / 批次 #2 基线，不自动扩样；新增 JD 走“（可选候选
   预检）→ 正式多次验收 → 人工审核 → finalize → 报告”的唯一正式主线；
2. 例外 1（JD 1～3）在本阶段内需决策（结构化豁免或重新验收）；
