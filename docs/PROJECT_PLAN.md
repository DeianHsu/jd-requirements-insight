...
| P0-7 正式生产主线收口 | 显式数据库目标；模型候选不入正式表；app finalize 定稿；报告只消费完整定稿批次；离线来源审计；真实链路 E2E | 🔵 待收口（例外 1：JD 1～3 豁免/重验待决——属本阶段关闭条件；机制部分已全部完成并经两轮评审验证） |

## 当前例外

| # | 所属阶段 | 例外内容 | 是否阻塞关闭 | 处置状态 |
|---|---|---|---|---|
| 1 | P0-3B / P0-7 | JD 1/2/3 正式抽取记录缺新定稿合同字段（机器分类 `unverified`，保留 legacy 人工审计结论；JD 4/5 为 `fully_bound`） | 对 P0-3B **不阻塞**（事后新合同标准回看，验证结论当时成立）；对 **P0-7 是关闭条件**（正式主线上游完整性未决） | **待决**：补结构化豁免记录（被豁免记录、原因、证据、批准人、允许消费的下游、是否仅限当前批次、新增数据禁止继续豁免）或重新验收补齐；决策前报告生成显式标注 provenance 风险 |
...
2. 例外 1（JD 1～3）决策是 **P0-7 收口的前提**（结构化豁免或重新验收），决策完成前 P0-7 保持 `🔵 待收口`；
