...
| P0-8 样本扩展（8 → 12 → 15 JD） | 先完成 JD 9～12 抽取/归并/报告，再处理 JD 13～15；逐批测量成本、稳定性与人工裁决量 | 进行中（8 JD 已关闭；12 JD 归并机器验收通过，待稳定性分析与人工裁决；15 为 MVP 固定终点） |
...
2. 进入扩样阶段（固定终点 15 JD）：8 JD 批次已关闭，JD 9～12 已完成
   抽取定稿，JD 1～12 全量归并机器验收 hard gate=0；下一步分析归并稳定性
   warning、完成必要人工裁决 → finalize consolidation → 12 JD 报告。
