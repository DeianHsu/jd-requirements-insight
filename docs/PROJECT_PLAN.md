...
| P0-8 样本扩展（8 → 12 → 15 JD） | 先完成 JD 9～12 抽取/归并/报告，再处理 JD 13～15；逐批测量成本、稳定性与人工裁决量 | 进行中（8 JD 已关闭；JD 9～12 抽取已定稿，待 12 JD 全量归并；15 为 MVP 固定终点） |
...
2. 进入扩样阶段（固定终点 15 JD）：8 JD 批次已关闭，JD 9～12 已完成
   抽取验收、人工审核和正式定稿；下一步执行 JD 1～12 全量归并验收，
   测量归并规模与人工裁决量 → 必要人工裁决 → finalize consolidation
   → 12 JD 报告。无真实阻塞再处理 JD 13～15，形成 15 JD 最终批次；
