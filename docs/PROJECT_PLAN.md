...
| P0-8 样本扩展（8 → 12 → 15 JD） | 先完成 JD 9～12 抽取/归并/报告，再处理 JD 13～15；逐批测量成本、稳定性与人工裁决量 | 进行中（8 JD 已关闭；12 JD 稳定性分析完成，人工审核材料已就绪；15 为 MVP 固定终点） |
...
2. 进入扩样阶段（固定终点 15 JD）：8 JD 批次已关闭，JD 9～12 已完成
   抽取定稿，JD 1～12 全量归并机器验收 hard gate=0，离线稳定性分析及
   public/private 人工审核材料已生成；下一步由外部 Reviewer 完成语义审核与
   必要裁决 → finalize consolidation → 12 JD 报告。
