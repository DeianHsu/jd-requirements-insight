...
| P0-8 样本扩展（8 → 12 → 15 JD） | 先完成 JD 9～12 抽取/归并/报告，再处理 JD 13～15；逐批测量成本、稳定性与人工裁决量 | 进行中（8 JD 已关闭；15 JD 输入已收齐；当前 8→12；15 为 MVP 固定终点） |
...
2. 进入扩样阶段（固定终点 15 JD）：8 JD 批次已关闭，JD 9～15 已按现行
   格式导入但尚未抽取；先处理 JD 9～12 形成 12 JD 批次，实测抽取成本、
   全量归并规模与人工裁决量 → 无真实阻塞再处理 JD 13～15，形成 15 JD
   最终批次。每批只执行正式主线（导入 → 验收 → 人工审核 → finalize
   extraction → 全量归并验收 → 必要人工裁决 → finalize
