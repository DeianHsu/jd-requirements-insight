...
> 只在标注候选人条件、检查要求原子化或解释抽取数据合同V2字段时读取本文。
...
“如”“例如”“等”只表示举例时，示例本身不能自动成为独立要求或任选组。不能只看到“等”字就判断为示例，应根据具体技术名在句中的语法作用区分：

- 完整的上位要求后使用“如/例如”或括号引出非穷举示例时，只标注上位要求。例如“至少精通一门主流后端开发语言（如Go、Java、C++、Python等）”标注为`主流后端开发语言`，`proficiency=expert`、`group_logic=standalone`；四种语言只是示例，不建立封闭的`any_of`组。
- 具体技术名被“熟悉”“掌握”“使用经验”等条件直接修饰时，即使后面带“等”，也保留为原子要求。例如“熟悉LangChain、AutoGen等主流Agent开发框架”分别标注`LangChain`和`AutoGen`；“等”只说明名单未穷尽，不能抹掉已经明确点名的技能。
- 具体模型名只用于修饰上位经验类型时，不单独标注模型。例如“有Llama、ChatGLM等大模型微调及应用落地经验”标注`大模型微调经验`和`大模型应用落地经验`，Llama与ChatGLM作为模型示例保留在证据中。
- 只有原文给出明确候选项，并表达“至少一种”“任一”“或”等有限任选关系时，才把候选项放入同一个`any_of`组。
...
- 没有明确年限时不猜测年限；
- 不根据常识修改重要程度或熟练度；
- 抽取阶段不把JD原词替换为标准要求项名称。

证据和人工标准答案规则见[DATASET_EVALUATION.md](DATASET_EVALUATION.md)。
