...
- **已持久化正式抽取结果：JD 1/2/3/4/5**（deepseek-v4-flash、
  `prompt:0.10|schema:3.0`，要求数 37/30/16/27/26，幂等已验证；
  JD 4/5 已绑定完整验收实验身份与来源文件指纹）；
- **已持久化正式归并批次：2 份**：
...
- 当前正式抽取来源绑定离线分类：JD 4/5 为 `fully_bound`；JD 1/2/3 的
  ...
  `unverified`。文档保留其既有人工审计结论；不回填、不重跑、不将其
  宣称为 `fully_bound`；
...
1. P0-7 已关闭，进入扩样阶段（固定终点 15 JD）：先新增 3 份形成
   8 JD 批次，实测抽取成本、全量归并规模与人工裁决量；无真实阻塞再
   增到 12 JD，最后 15 JD；不扩展到 20 JD；
2. 每批只执行现有正式主线：导入 → 抽取验收 → 人工审核 → finalize
   extraction → 全量归并验收 → 必要人工裁决 → finalize consolidation
   → 报告；新增 JD 禁止使用 JD 1/2/3 的历史豁免；
