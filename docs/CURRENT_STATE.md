...
- **归并预检分层采样**（`run_small_scale_precheck`）：由按
  ...
  摘要；
- **大模块 4 已完成（2026-08-05，含已授权付费阶段）**：
  - JD 4/5 抽取验收：hard gate=0（JD4: 27/28/26 条，JD5: 26/26/26 条），
    漂移均为命名/粒度/类别边界差异，无幻觉/职责泄漏；批准 run0 并
    原样定稿（正式记录 ID 4/5，回读逐项一致，幂等）；
  - 5 JD 归并预检（分层采样 80 条）：coverage=100%、结构违规=0，
    新旧条件进入合理 cluster；
  - 5 JD 完整归并验收（136 实例，3 独立 + 顺序变形）：hard gate=0、
    顺序变形合同通过；Jaccard 39~67% 为诊断指标；
  - 增量稳定性：旧 83 条限制比较——旧 14 对中 9 对被模型拆开（波动）、
    7 个旧 singleton 被吸收、10 个新合并对、must-link 破坏 8 个、
    cannot-link 0 破坏；旧裁决全部维持（无新证据调整），新增
    must-link 5 组（专业 2-85、算法 70-90、热情 30-107-126、
    工程素养 40-92-114、协作 110 入族）、unresolved 3 组（any_of
    等价替代保持分开）；
  - 新正式批次 **id=2**（job_ids=1,2,3,4,5、136 条、103 canonical、
    审核指纹 c04879c8…），与旧批次 id=1 并存（增量基线）；离线
    一致性验证 NONE、幂等；
  - 5 JD 报告 `reports/P0-5/market-report-5jd.md`（私有）：团队协作
    4/5 JD、跨 JD 共同 19、长尾 84、样本声明动态 5 JD、确定性一致；
  - 3→5 对比摘要 `reports/P0-4/module4-3to5-comparison.json`；
  - **是否进入 6～8 JD**：新增抽取无系统性缺陷、验收可原样定稿、
    归并合同通过、旧核心关系经裁决保持稳定、新增边界 8 组可人工
    审核、输入 136 条成本可接受、报告无结构问题——满足进入条件，
    但按模块边界**不自动扩展**，等待下一模块指令。
