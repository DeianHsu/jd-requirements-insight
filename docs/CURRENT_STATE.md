...
- 9 份真实 JD 已导入 `data/jd_skill_insight.db`（JD 1～9，重复导入幂等跳过）；
- JD 9（百林科｜AI Agent 工程师）已按现行 Markdown/frontmatter 格式保存并
  导入，尚未执行付费抽取或创建正式抽取记录；
...
1. **8 JD 批次已正式关闭**（批次 #3 定稿 + 8 JD 报告生成，provenance
   文案已修正为引用 P0-7 豁免记录）；进入 **8 → 12 JD 扩样**：JD 9
   已导入、尚未抽取，用户再提供 3 份真实 JD 形成 JD 9～12 增量 →
   抽取验收 → 人工审核 → finalize
