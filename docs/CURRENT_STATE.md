...
- 当前正式抽取来源绑定离线分类：JD 4/5 为 `fully_bound`；JD 1/2/3 的
  ...
  `unverified`。文档保留其既有人工审计结论；**决策前不回填、不重跑、
  不将其宣称为 `fully_bound`**；处置待决（结构化豁免记录或重新验收），
  是 P0-7 收口前提（见 PROJECT_PLAN「当前例外」例外 1）；
...
3. 例外 1（JD 1～3）处置待决：结构化豁免记录或重新验收，决策是
   P0-7 收口前提（PROJECT_PLAN「当前例外」）。
