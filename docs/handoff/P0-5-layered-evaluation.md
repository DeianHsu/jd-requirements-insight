...
部分完成。完整JD人工标准答案校验和困难样例分层评测已经实现；`evaluate-cases`支持开发集、回归集和验证集。尚缺完整验证集、P0-4正式完整验收指标和证据支持性人工复核闭环；P0-4连续3次验收失败后已暂停。
...
当前工作树全量110项测试和Ruff通过。已记录的抽取阶段结果包括开发集要求名称F1为100%、回归集职责F1为78.26%，两组证据存在率均为100%。P0-4已提供按显式批次ID运行的正式离线评测命令；历史批次5的关系Precision/Recall/F1为66.67%/80%/72.73%，但新Prompt正式完整验收连续失败3次，修复后的全量指标尚未产生。

# 未完成事项与已知问题

- 新验证集尚未建立，现有结果不能证明稳定泛化能力。
- 完整JD人工标准答案数量未达到计划规模。
- 要求映射准确率和关系Precision/Recall/F1已在P0-4实现但尚未接入；证据支持率人工复核仍未实现。
