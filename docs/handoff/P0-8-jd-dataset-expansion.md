...
related_decisions: ["DEC-003", "DEC-013", "DEC-019"]
glossary_terms: ["原始JD", "结构化抽取", "人工标准答案", "归并输入指纹"]
...
- 扩充后的目标JD必须全部完成同一抽取器版本，才能进入一次完整归并；缺少目标版本时不得生成部分`all`批次。
...
当前工作树全量110项测试和Ruff通过。导入测试覆盖元数据解析、空白规范化、幂等导入和文件级错误隔离；P0-4范围完整性测试会拒绝扩充后尚未完成共同版本抽取的JD集合。

# 未完成事项与已知问题

- 样本规模尚未达到15～20份。
- 尚缺完整的数据来源与筛选记录闭环。
- 批量真实抽取依赖P0-3进一步稳定，以及P0-4从连续3次验收失败的暂停状态恢复。
