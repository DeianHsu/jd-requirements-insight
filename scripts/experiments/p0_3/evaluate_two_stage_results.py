"""该模块离线评测P0-3两段式抽取结果，并生成开发集与回归集对照报告。"""
...
def build_report(
    cases: dict[str, Any], predictions: dict[str, JobExtractionResult]
) -> str:
    ...
    for split in ("development", "regression"):
        metrics = evaluate_annotation_cases(
            cases, predictions, dataset_split=split
        )
    ...
    lines.extend(
        [
            ...
            "|---|---:|---:|---:|---:|",
            "| 要求名称F1 | 100% | - | 见上 | 见上 |",
            "| 职责名称F1 | 100% | 78.26% | 见上 | 见上 |",
            "| 数量一致 | 10/10 | 2/5 | 见上 | 见上 |",
        ]
    )
    return "\n".join(lines)
...
def main() -> None:
    ...
    args.output.write_text(
        build_report(cases, predictions),
        encoding="utf-8",
    )
