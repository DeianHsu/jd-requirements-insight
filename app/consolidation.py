...
【等价判定标准】
1. 两个实例描述的是同一个招聘条件（同一技能、同一经验门槛、同一学历门槛），只是措辞、表述形式或修饰词不同，必须归并为同一标准要求项；跨JD出现同一条件时不得因措辞不同拆成多个canonical。
2. 正例："能力甲应用开发相关知识"与"能力甲应用开发技术原理"是同一条件；"熟悉能力甲"与"掌握能力甲"是同一条件（熟练度差异不改变条件身份）；"使用能力甲的经验"与"具备能力甲使用经验"是同一条件；"有技术甲应用落地经验"与"有技术甲工程化落地经验"是同一条件；"能力乙"与"能力乙技术"若证据指向同一技能也是同一条件。
3. 反例："能力甲开发经验"与"能力甲使用经验"若证据分别指向开发岗位经验与工具使用经验，是不同条件；"能力甲框架"与"能力甲"若前者特指框架本身而后者是泛指技能，是不同条件；同一句并列列举的不同技术名（如"熟悉能力甲、能力乙等框架"拆出的"能力甲"与"能力乙"）是两条独立条件，不得在归并时合并为一个canonical或改写成上位概念。
4. 判定依据是证据上下文中的招聘门槛语义，不是关键词重合度；不确定时优先依据证据判断，证据充分指向同一门槛就合并，证据确实矛盾才保持singleton。
...
def load_consolidation_selection(
    ...
    # 当前主线只消费 v0.10 + Schema V3：显式指定或自动选中的旧版本都拒绝。
